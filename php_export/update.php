...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
 ...
 $updateStmt = $pdo->prepare("UPDATE products SET name = ?, category = ?, price = ?, stock = ?, image = ?, description = ? WHERE id = ?");
 $updateStmt->execute([$name, $category, $price, $stock, $image, $description, $id]);
...
<head>
 ...
 <title>Edit Product - Mafikul's Store</title>
...
<div class="fixed inset-0 -z-50 overflow-hidden pointer-events-none transition-colors duration-300">
 ...
 <div class="mb-8">
 <h1 class="font-serif text-3xl font-bold tracking-tight text-gray-900 dark:text-white">Edit Product</h1>
 <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Update product information for your catalog.</p>
 </div>
 ...
 disabled
 class="block w-full rounded-md border-0 py-2 px-3 text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-950 shadow-sm ring-1 ring-inset ring-gray-300 dark:ring-gray-700 sm:text-sm sm:leading-6 cursor-not-allowed"
 >
 </div>
 </div>
 </div>
 ...
 <div class="mt-2">
 <?php $categories = ['Clothing', 'Electronics', 'Computers', 'Accessories', 'Audio', 'Wearables', 'Home', 'Kitchen', 'Stationery', 'Apparel', 'Footwear', 'Sports', 'Beauty', 'Health', 'Uncategorized']; ?>
 <select
 id="category"
 name="category"
 required
 ...
 <div>
                    ...
                    >
                        <div class="text-center w-full">
                            <div id="image-preview" class="mx-auto flex justify-center w-full">
                                <img id="preview-img" src="<?php echo htmlspecialchars($product["image"]); ?>" alt="Preview" class="h-64 object-cover rounded-md" />
                            </div>
